...
impl MempoolAlbatrossHandler {
    ...
    fn parse_address(value: &JsonValue, kind: &str) -> Result<Address, JsonValue> {
        JsonValue::as_str(value)
            ...
            .and_then(|it| {
                Address::from_user_friendly_address(it)
    ...
    pub(crate) fn stake(&self, params: &[JsonValue]) -> Result<JsonValue, JsonValue> {
        let validator_key = params
            ...
            .and_then(|it| {
                CompressedPublicKey::deserialize_from_vec(&it)
                    .map_err(|_| object! {"message" => "Invalid public key"})
            })?;
        let proof_of_knowledge = params
            .get(1)
            .and_then(JsonValue::as_str)
            ...
            .and_then(|it| {
                CompressedSignature::deserialize_from_vec(&it)
        ...
        let mut tx = Transaction::new_extended(
            staker_address,
            AccountType::Basic, // sender
            staking_contract.clone(),
            AccountType::Staking, // recipient
            amount,
            fee,                             // amount, fee
    ...
    pub(crate) fn retire(&self, params: &[JsonValue]) -> Result<JsonValue, JsonValue> {
        ...
        let mut tx = Transaction::new_extended(
            ...
            AccountType::Staking,
            amount,
            Coin::try_from(0).unwrap(),    // amount, fee
            vec![],                        // data
            self.mempool.current_height(), // validity_start_height
            network_id,                    // network_id
        );
    ...
    pub(crate) fn unstake(&self, params: &[JsonValue]) -> Result<JsonValue, JsonValue> {
        ...
        let mut tx = Transaction::new_extended(
            staking_contract.clone(),
            AccountType::Staking, // sender
            staker_address,
            AccountType::Basic, // recipient
            amount,
            Coin::try_from(0).unwrap(),    // amount, fee
            vec![],                        // data
            self.mempool.current_height(), // validity_start_height
