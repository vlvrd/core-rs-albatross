...
impl BlockchainState {
    ...
    pub fn current_slashed_set(&self) -> BitSet {
        self.reward_registry
            .slashed_set(policy::epoch_at(self.block_number()), None)
    }

    pub fn last_slashed_set(&self) -> BitSet {
        self.reward_registry
            .slashed_set(policy::epoch_at(self.block_number()) - 1, None)
    }
...
impl Blockchain {
    ...
    fn extend(
        ...
        if let Block::Macro(ref mut macro_block) = &mut chain_info.head {
            ...
            let computed_extrinsics =
                MacroExtrinsics::from_stake_slots_and_slashed_set(slots.stake_slots, slashed_set);
                ...
                Block::Micro(ref micro_block) => {
                    let result = if !cache_txn.contains_any(&fork_block.1.head) {
                        state
                            ...
                            .and_then(|_| {
                                self.commit_accounts(&state, &mut write_txn, &fork_block.1.head)
                            })
                    } else {
        ...
        match block {
            Block::Macro(ref macro_block) => {
                ...
                let view_changes = ViewChanges::new(
                    ...
                    state.main_chain.head.next_view_number(),
        ...
        (view_changes.first_view_number..view_changes.last_view_number)
            .map(|view_number| {
                ...
                debug!(
                    ...
                    producer.staker_address().to_user_friendly_address()
                );
    ...
    pub fn slashed_set_at(
        ...
        s.reward_registry
            .slashed_set_at(epoch_number, block_number, None)
    }
