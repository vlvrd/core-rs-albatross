...
impl PartialEq for ActiveStake {
    fn eq(&self, other: &ActiveStake) -> bool {
        self.balance == other.balance && self.staker_address == other.staker_address
    }
}
...
impl StakingContract {
    ...
    pub fn get_active_balance(&self, staker_address: &Address) -> Coin {
        self.active_stake_by_address
            .get(staker_address)
            .map(|stake| stake.balance)
            .unwrap_or(Coin::ZERO)
    }
    ...
    pub fn stake(
        ...
        validator_key: BlsPublicKey,
        reward_address: Option<Address>,
    ) -> Result<Option<ActiveStakeReceipt>, AccountError> {
        ...
        if let Some(active_stake) = self.active_stake_by_address.remove(staker_address) {
            ...
            self.active_stake_sorted
                .insert(Arc::clone(&new_active_stake));
            self.active_stake_by_address
                .insert(staker_address.clone(), new_active_stake);
            ...
            self.active_stake_by_address
                .insert(staker_address.clone(), stake);
    ...
    fn revert_stake(
        ...
        staker_address: &Address,
        value: Coin,
        receipt: Option<ActiveStakeReceipt>,
    ) -> Result<(), AccountError> {
    ...
    fn unpark_sender(
        ...
        // Then deduct fee.
        let new_active_stake =
            Arc::new(active_stake.with_balance(Account::balance_sub(active_stake.balance, fee)?));
    ...
    fn revert_unpark_sender(
        ...
        // Then deduct fee.
        let new_active_stake =
            Arc::new(active_stake.with_balance(Account::balance_add(active_stake.balance, fee)?));
...
impl AccountTransactionInteraction for StakingContract {
        ...
        if transaction.sender != transaction.recipient {
            ...
            match ty {
                StakingTransactionType::Retire => {
                    // Retire transaction.
                    Ok(self
                        .retire_recipient(&staker_address, transaction.value, block_height)?
                        .map(|receipt| receipt.serialize_to_vec()))
                }
                StakingTransactionType::Unpark => Ok(Some(
                    self.unpark_recipient(&staker_address, transaction.value)?
                        .serialize_to_vec(),
                )),
            }
        }
    ...
    fn check_outgoing_transaction(
        ...
        if transaction.sender != transaction.recipient {
            ...
            // Check unstake delay.
            if block_height
                < policy::macro_block_after(inactive_stake.retire_time) + policy::UNSTAKING_DELAY
            {
                return Err(AccountError::InvalidForSender);
            }
            ...
            match ty {
                ...
                StakingTransactionType::Unpark => {
                    ...
                    if !self.current_epoch_parking.contains(&staker_address)
                        && !self.previous_epoch_parking.contains(&staker_address)
                    {
                        return Err(AccountError::InvalidForSender);
                    }
                    Ok(())
        ...
        if transaction.sender != transaction.recipient {
            ...
            match ty {
                StakingTransactionType::Retire => {
                    // Retire transaction.
                    Ok(self
                        .retire_sender(&staker_address, transaction.total_value()?, block_height)?
                        .map(|receipt| receipt.serialize_to_vec()))
                }
                StakingTransactionType::Unpark => {
                    self.unpark_sender(
    ...
    fn revert_outgoing_transaction(
        ...
        if transaction.sender != transaction.recipient {
            ...
            match ty {
                ...
                StakingTransactionType::Unpark => self.revert_unpark_sender(
                    &staker_address,
                    transaction.total_value()?,
                    transaction.fee,
                ),
...
impl Serialize for StakingContract {
    fn serialize<W: WriteBytesExt>(&self, writer: &mut W) -> Result<usize, SerializingError> {
        ...
        for active_stake in self.active_stake_sorted.iter() {
            let inactive_stake = self
                .inactive_stake_by_address
                .get(&active_stake.staker_address);
...
impl Deserialize for StakingContract {
    fn deserialize<R: ReadBytesExt>(reader: &mut R) -> Result<Self, SerializingError> {
        ...
        for _ in 0..num_active_stakes {
            ...
            active_stake_by_address.insert(
                active_stake.staker_address.clone(),
                Arc::clone(&active_stake),
            );
        ...
        let current_epoch_parking: HashSet<Address> =
            DeserializeWithLength::deserialize::<u32, _>(reader)?;
        let last_epoch_parking: HashSet<Address> =
            DeserializeWithLength::deserialize::<u32, _>(reader)?;
